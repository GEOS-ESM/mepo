...
class GitRepository(object):
    ...
    def push(self,tags,hashes):
        if tags:
            ...
            current_hash = shellcmd.run(shlex.split(cmd), output=True).strip()
            ...
            push_output = shellcmd.run(shlex.split(cmd), output=True).strip()
            print(push_output)

            print(f'Deleting {tmp_branch} from remote')
            cmd = self.__git + ' push {} --delete {}'.format(self.__remote,tmp_branch)
            push_del_output = shellcmd.run(shlex.split(cmd), output=True).strip()
